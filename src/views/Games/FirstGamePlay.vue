<template>
    ...
    <div :class="'first-game'">
        ...
        <div class="first-game__menu">
            <div class="first-game__tag" v-for="_, ind in new Array(currentWord + 1).fill(0)">
        ...
        <div class="first-game__tab">
            <div class="slot">
                <div class="slot__top">
                    <div v-if="allWords.length > 0" class="slot__place" v-for="s, ind in (allWords[currentWord]).length"
                        :style="{ top: ind > 4 ? '58px' : 0, left: ((ind % 5) * 66) + 'px' }"></div>
                    <div v-if="generatedLetters.length > 0" class="slot__let" v-for="lett, index in generatedLetters"
                        :style="getPosition(lett, index)" @click="moveLetter(lett, index)">{{
                            lett }}</div>
                </div>
            </div>
        </div>
...
watch(() => answer.value.length, (newVal, oldVal) => {
...
const moveLetter = (letter, index) => {
    if (answer.value.find(item => item.letter === letter && item.index === index)) {
...
const startGame = () => {
    ...
    function generateLetters() {
        console.log('allWords.value[currentWord.value]', allWords.value[currentWord.value])

        const letters = allWords.value.length > 0 ? [...allWords.value[currentWord.value].toUpperCase()] : [] // включаем все буквы слова (с дубликатами)
